...
.mara-watched .wopt input[type="radio"] { accent-color: var(--ember); flex-shrink: 0; }
.mara-watched .wdate { align-self: flex-start; background: var(--ink);
  border: 1px solid var(--rule-strong); border-radius: var(--r-2); color: var(--bone);
  font-family: var(--font-ui); font-size: 12px; padding: 6px 8px; }
.mara-watched .wdate:focus { outline: none; border-color: var(--ember); }
